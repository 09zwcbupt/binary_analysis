/*
 * This tracer.c locates the symbol table by finding and referencing
 * the SHT_SYMTAB type section header, so it will not work on executables
 * that have been stripped of the SHT_SYMTAB symbol table (although
 * they may have SHT_DYNSYM).
 * */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
int main(int argc, char **argv, char **envp) {
    ...
    // update: original code uses strcmp, which looks like a issue
    ...
    for( i = 0; i < h->ehdr->e_shnum; i++ ) {
        ...
        if( h->shdr[ i ].sh_type == SHT_SYMTAB ) {
            ...
            for( j = 0; j < h->shdr[ i ].sh_size / sizeof( Elf64_Sym ); j++ ) {
               //printf("symble name: %s %s\n", &strtab[ symtab->st_name ], symname );
